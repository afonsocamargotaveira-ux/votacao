        ...
        if resultados:
            ...
            with st.expander(
                ...
                st.caption(
                    ...
                    "e verifica como o(a) deputado(a) votou em cada uma. Recomenda-se períodos "
                    "de até 60 dias para não demorar muito."
                )
                colp1, colp2 = st.columns(2)
                with colp1:
                    data_ini_dep = st.date_input(
                        "Data inicial", value=date.today() - timedelta(days=30), key="data_ini_dep"
                    )
                with colp2:
                    data_fim_dep = st.date_input(
                        "Data final", value=date.today(), key="data_fim_dep"
                    )
